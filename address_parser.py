...
def extract_address_metadata(input_str: str):
    ...
    df = pd.read_csv("malaysia-postcodes-location-mukim-district-state.csv",dtype='str')
        ...
        if district_found:                                                                                                                              
            for district in district_found:
                    ...
                    if mukim_found:
                        for mukim in mukim_found:
                            ...
                            if mukim != district:
                                ...
                                location_found = [x for x in [l for l in location if l.lower() in input_str.lower()] if x in [x.strip() for x in input_str.lower().strip().split(',')]]
                                # 8) [1248]
                                if location_found:
                ...
                # 2b)
                    ...
                    mukim_found = [l for l in mukim if l in input_str.lower()]
    ...
    state_found = [s for s in state if s in input_str.lower()] # case len = 1 , case len = 0 , case len > 1
    district_found = [d for d in district if d in input_str.lower()]
    mukim_found = [m for m in mukim if m.lower() in input_str.lower()]
    location_found = [l for l in location if l in input_str.lower()]  # Convert float to string
    '''
    ...
    if len(state_found) == 1:                                                                                                                 
        ...
        mukim_found = [x for x in [m for m in mukim if m.lower() in input_str.lower()] if x in [x.strip() for x in input_str.lower().strip().split(',')]]
    ...
    #'''
    if len(postcode_found) > 0:
        postcode_update = postcode_update + postcode_found
    elif len(postcode_found) == 0:
        postcode = pd.read_csv("malaysia-postcodes-location-mukim-district-state.csv",dtype='str')

        # filter the postcode
        if (len(state_update) == 1) and (len(district_update) == 1) and (len(mukim_update) == 1) and (len(location_update) == 1):
            postcode = postcode[postcode['state'].str.lower() == state_update[0]]
            postcode = postcode[postcode['district'].str.lower() == district_update[0]]
            postcode = postcode[postcode['mukim'].str.lower() == mukim_update[0]]
            postcode = postcode[postcode['location'].str.lower() == location_update[0]]

            postcode = postcode[["postcode"]].dropna().drop_duplicates()['postcode'].str.lower().tolist()
            postcode = [x.strip() for x in list(set(postcode)) for x in x.split(",")]
            postcode_update = postcode
        
        elif (len(state_update) == 1) and (len(district_update) == 1) and (len(location_update) == 1):
            postcode = postcode[postcode['state'].str.lower() == state_update[0]]
            postcode = postcode[postcode['district'].str.lower() == district_update[0]]
            postcode = postcode[postcode['location'].str.lower() == location_update[0]]
        ...
        elif (len(state_update) == 1) and (len(district_update) == 1):
            postcode = postcode[postcode['state'].str.lower() == state_update[0]]
            postcode = postcode[postcode['district'].str.lower() == district_update[0]]
        ...
        elif (len(state_update) == 1) and (len(location_update) == 1):
            postcode = postcode[postcode['state'].str.lower() == state_update[0]]
            postcode = postcode[postcode['location'].str.lower() == location_update[0]]
    ...
    if len(state_found) == 0:
        ...
        if (len(district_found) == 1):
            state = df[df['district'].str.lower() == district_found[0]][["state"]].dropna().drop_duplicates()['state'].str.lower().tolist()
            postcode = df[df['district'].str.lower() == district_found[0]][["postcode"]].dropna().drop_duplicates()['postcode'].str.lower().tolist()
            district = df[df['district'].str.lower() == district_found[0]][["district"]].dropna().drop_duplicates()['district'].str.lower().tolist()

            state_update = state_update+state
            postcode_update = postcode_update+postcode
            district_update = district_update+district
        '''	
        elif (len(district_update) > 0) and (len(mukim_update) > 0) and (len(location_update) > 0):
            state = state[state['district'].str.lower() == district_found[0]]
            state = state[state['mukim'].str.lower() == mukim_update[0]]
            state = state[state['location'].str.lower() == location_update[0]]
            state = state[["state"]].dropna().drop_duplicates()['state'].str.lower().tolist()
            state = [x.strip() for x in list(set(state)) for x in x.split(",")]
            state_update = state

        elif (len(district_update) > 0) and (len(mukim_update) > 0):
            state = state[state['district'].str.lower() == district_found[0]]
            state = state[state['mukim'].str.lower() == mukim_update[0]]
            state = state[["state"]].dropna().drop_duplicates()['state'].str.lower().tolist()
            state = [x.strip() for x in list(set(state)) for x in x.split(",")]
        ...
        elif (len(district_update) > 0) and (len(location_update) > 0):
            state = state[state['district'].str.lower() == district_found[0]]
            state = state[state['location'].str.lower() == location_update[0]]
            state = state[["state"]].dropna().drop_duplicates()['state'].str.lower().tolist()
            state = [x.strip() for x in list(set(state)) for x in x.split(",")]
        ...
        elif (len(mukim_update) > 0) and (len(location_update) > 0):
            state = state[state['mukim'].str.lower() == mukim_update[0]]
            state = state[state['location'].str.lower() == location_update[0]]
            state = state[["state"]].dropna().drop_duplicates()['state'].str.lower().tolist()
            state = [x.strip() for x in list(set(state)) for x in x.split(",")]
